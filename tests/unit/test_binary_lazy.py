"""Verify binary Blueprint has no top-level tool imports."""
import ast
import os
                ...
                if isinstance(child, ast.ImportFrom):
                    if child.module and child.module.startswith("tools."):
                        raise AssertionError(
                            f"Top-level try import found in binary.py: 'from {child.module} import ...'"
                        )
        if isinstance(node, (ast.Import, ast.ImportFrom)):
            if hasattr(node, "module") and node.module and node.module.startswith("tools."):
                raise AssertionError(
                    f"Top-level tool import found in binary.py: 'from {node.module} import ...'"
                )
